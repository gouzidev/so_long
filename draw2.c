...
/*   Updated: 2024/03/05 20:51:36 by sgouzi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_count_str(t_data *data)
{
	...
	movement_count_str = ft_itoa(data->movement_counter);
...
void	handle_player(t_data *data, int i, int j, int *p_count)
{
	...
	if (data->mapo->pd == 0)
	{
		if (data->mapo->p_frame == 0)
			put_player_right_img(data, i, j);
		else
			put_player_right_img(data, i, j);
	}
	else
