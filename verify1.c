...
void	verify_exit_count(t_data *data)
{
	...
	while (data->mapo->map[i])
	{
		...
		while (data->mapo->map[i][j])
		{
			...
			{
				if (++exit_count > 1)
					close_window_msg(data, "more than one door !");
			}
			j++;
		}
		i++;
...
void	verify_enemy_count(t_data *data)
{
	...
	while (data->mapo->map[i])
	{
		...
		while (data->mapo->map[i][j])
		{
			...
			{
				if (++enemy_count > 1)
					close_window_msg(data, "more than one enemy !");
			}
			j++;
		}
		i++;
	...
	if (j < 3 || i < 3)
		close_window_msg(data, "map too small \n");
}
		...
		while (data->mapo->map[i][j])
		{
			if (data->mapo->map[i][j])
			{
				if (ft_strchr("PENC01", data->mapo->map[i][j]) == 0)
					close_window_msg(data, "invalid map character\n");
			}
			j++;
		}
		i++;
